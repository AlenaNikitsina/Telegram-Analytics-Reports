...
connection = {
            ...
            #извлекаем данные из таблиц за 7 дней  по работе всего приложения как единого целого   
q_dau_all =  '''
    SELECT date, 
...
def prihodko_app_report():
    ...
    def report_text(data):
        ...
        ❤️Лайки: {likes} ({to_likes_day_ago:+.2%}, {to_likes_week_ago:+.2%})
        🎯CTR: {ctr} ({to_ctr_day_ago:+.2%}, {to_ctr_week_ago:+.2%})

        МЕССЕНДЖЕР:
        👤DAU: {users_msg} ({to_users_msg_day_ago:+.2%}, {to_users_msg_week_ago:+.2%})
        ✉️Количество сообщений: {count_msg} ({to_count_msg_day_ago:+.2%}, {to_count_msg_week_ago:+.2%})
        '''
    ...
    @task()
    def send(report, plot_object_app, plot_object_lm, chat_id):
        # инициализируем бота
    ...
    send(report, plot_object_app, plot_object_lm, chat_id=-1002614297220)


prihodko_app_report = prihodko_app_report()
